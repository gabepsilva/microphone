...
This module is deliberately standalone: it never imports ``voice-codex.py`` and
owns no audio, transcription, or Codex logic. It renders state and forwards
user intent. Everything it displays arrives through :class:`VoiceCodexTUI`, a
thread-safe facade, and everything the user does leaves through
:class:`TuiHooks` callbacks. That keeps it pluggable — the real script can
construct a ``VoiceCodexTUI``, hand it to its display/listener layer, and use it
in place of the ANSI ``TranscriptDisplay``.
...
class SessionState:
    ...
    them: Channel = field(default_factory=lambda: Channel("them"))
    out_device: str = "—"

    codex_model: str = "gpt-5.6-luna"
    # Placeholder roster for the picker — the host replaces it with the models
    # its Codex install actually offers via VoiceCodexTUI.set_models().
...
def _row(stamp: str, source: str, body: RenderableType) -> Table:
    grid = Table.grid(padding=(0, 1))
    grid.add_column(width=8, style="#5a6068", no_wrap=True)
    grid.add_column(width=11, no_wrap=True)
    grid.add_column(ratio=1, overflow="fold")
    label = Text(source, style=SOURCE_STYLES.get(source, "#5a6068"))
    grid.add_row(stamp, label, body)
    return grid


def render_entry(entry: Entry) -> RenderableType:
    if entry.kind == "note":
        return _row(entry.stamp, "·", Text(entry.text, style="#6f757e"))

    if entry.kind == "command":
        lines: list[RenderableType] = [
            Text(f"$ {entry.text}", style="#9aa3ad"),
        ]
        lines.extend(Text(line, style="#6f757e") for line in entry.output)
        if entry.exit_code is not None:
            lines.append(Text(f"[command exit: {entry.exit_code}]", style="#5a6068"))
        return Group(*lines)
    ...
    return _row(entry.stamp, entry.source, body)
...
class EntryRow(Static):
    """A single transcript entry. Re-renders in place while streaming."""

    def __init__(self, entry: Entry) -> None:
        super().__init__(render_entry(entry))
        self.entry = entry
        if entry.kind == "command":
            self.add_class("command")

    def sync(self) -> None:
        self.update(render_entry(self.entry))


class Sidebar(Vertical):
    """Live status column: static text plus the policy and model pickers."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="policy-row"):
            yield Static("respond", id="policy-label")
            yield Select(
        ...
        with Horizontal(id="model-row"):
            ...
            yield Select(
                [(model, model) for model in self.state.codex_models],
    ...
    @staticmethod
    def _option_label(selector: str, value: str) -> str:
        return POLICIES[value] if selector == "#policy-select" else value
...
class VoiceCodexApp(App):
    ...
    #model-row, #policy-row { height: 1; }
    #model-label { width: 6; color: #6f757e; }
    #policy-label { width: 8; color: #6f757e; }
    #sidebar Select {
        width: 1fr;
        margin: 0;
        background: #0f1113;
    ...
    def action_toggle_tts(self) -> None:
        ...
        if self.hooks.on_tts:
            self.hooks.on_tts(self.state.tts_enabled)
...
class VoiceCodexTUI:
    ...
    def commit(self, source: str, text: str) -> None:
        """Append a finished turn and clear the live line."""
        self.state.partial_source = ""
        self.state.partial_text = ""
        self._call(self._commit_impl, source, text)

    def _commit_impl(self, source: str, text: str) -> None:
        self.app.add_entry(Entry(kind="speech", source=source, text=text))
    ...
    def command_output(self, line: str) -> None:
        self._call(self._command_output_impl, line)
    ...
    def set_models(self, models: list[str]) -> None:
        """Replace the model picker's roster (keeps the current model if listed)."""
        self.state.codex_models = list(models)
        if self.state.codex_model not in self.state.codex_models:
            self.state.codex_models.insert(0, self.state.codex_model)
        self._call(self.app.refresh_sidebar)
...
# --------------------------------------------------------------------------
# Entry point — an empty session driven entirely through the facade
# --------------------------------------------------------------------------


def main() -> None:
    argparse.ArgumentParser(description=__doc__).parse_args()
    VoiceCodexTUI(SessionState()).run()
