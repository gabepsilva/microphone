...
- **Electron per-file coverage floors** (`electron/coverage_floors.json` +
  ...
  `electron-coverage`): Bun writes lcov; the gate walks every
  `electron/src/**/*.ts` file so an unimported module cannot escape as
  "not measured". Floors record what the suite measured when the gate landed
  and only ratchet upward (`make ratchet` reads the same JSON).
  `main.ts` is listed under `unmeasured_entrypoints` because unit tests must
  not launch a `BrowserWindow` (#97); growing that exemption list is a
  ratchet failure. Planted proofs live in `electron/tests/coverage_gate.test.ts`.
  Mutation testing on the Electron protocol surface stays deferred with
  measured cost: pure protocol code is 163 lines
  (`actions.ts` + `channels.ts` + `dispatch_allowlist.ts`), already at 100%
  line coverage with payload-shape assertions in
