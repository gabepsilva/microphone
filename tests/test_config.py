...
def test_a_config_that_cannot_be_written_is_reported(tmp_path) -> None:
    unwritable = tmp_path / "missing-directory" / "tagalong.yaml"
...
def test_missing_ok_does_not_hide_other_read_failures(tmp_path) -> None:
    directory = tmp_path / "tagalong.yaml"
    directory.mkdir()
...
def test_an_unwritable_config_names_the_file_it_could_not_write(tmp_path) -> None:
    ...
    with pytest.raises(
        RuntimeError, match=r"Could not save startup config '[^']*tagalong\.yaml'"
    ):
        save_startup_config(unwritable, {"microphone": "Yeti"})
...
def test_a_key_the_file_cannot_hold_is_refused() -> None:
    ...
    config = StartupConfigFile("tagalong.yaml", stored(), save=RecordingSave())
