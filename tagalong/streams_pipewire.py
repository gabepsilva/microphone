#!/usr/bin/env python3
"""PipeWire implementation of the application-stream capture backend.
...
class StreamTap:
    ...
    def process_options(self):
        """Keep PipeWire diagnostics from filling an undrained stderr pipe."""
        return {"stdin": subprocess.DEVNULL, "stderr": subprocess.DEVNULL}

    def wait_ready(self, process, timeout):
        """Use the short PipeWire startup check, within the shared timeout."""
        time.sleep(min(self.STARTUP_GRACE_SECONDS, timeout))
        return (
            f"recorder exited with code {process.poll()}"
            if process.poll() is not None
            else None
        )

    def attach(self, _process):
        """PipeWire has no helper control channel to attach."""
        return
