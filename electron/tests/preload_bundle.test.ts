...
describe("sandboxed preload bundle", () => {
  it(
    "build emits a preload with no relative requires",
    () => {
    ...
    // Full `bun run build` (tsc + preload bundle) under parallel `make ci`
    // can exceed Bun's default 5s when the machine is saturated.
    { timeout: 60_000 },
  );
